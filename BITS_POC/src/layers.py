...
from BITS_POC.util.common import JTensor
from BITS_POC.util.common import JavaValue
from BITS_POC.util.common import callBigDlFunc
from BITS_POC.util.common import callJavaFunc
from BITS_POC.util.common import get_spark_context
from BITS_POC.util.common import to_list
from BITS_POC.util.common import INTMAX, INTMIN, DOUBLEMAX
from BITS_POC.util.common import get_activation_by_name
from BITS_POC.optim.optimizer import L1Regularizer, L2Regularizer, L1L2Regularizer
from py4j.java_gateway import JavaObject
from pyspark.rdd import RDD
from BITS_POC.transform.vision.image import ImageFrame
...
class Node(JavaValue):
    ...
    def __init__(self, jvalue, bigdl_type, *args):
        self.value = (
            jvalue
            if jvalue
            else callBigDlFunc(bigdl_type, self.jvm_class_constructor(), *args)
        )
...
class SharedStaticUtils:
    ...
    def of(jvalue, bigdl_type="float"):
        ...
        jname = callBigDlFunc(bigdl_type, "getRealClassNameOfJValue", jvalue)

        jpackage_name = ".".join(jname.split(".")[:-1])
        pclass_name = get_py_name(jname.split(".")[-1])

        if (
            "com.intel.analytics.bigdl.nn.keras.Model" == jname
            or "com.intel.analytics.bigdl.nn.keras.Sequential" == jname
        ):
...
class Layer(JavaValue, SharedStaticUtils):
    ...
    def forward(self, input):
        ...
        output = callBigDlFunc(
            self.bigdl_type, "modelForward", self.value, jinput, input_is_table
        )
    ...
    def parameters(self):
        ...
        """
        name_to_params = callBigDlFunc(
            self.bigdl_type, "modelGetParameters", self.value
        )
        ...
        def to_ndarray(params):
            return dict(
                (
                    param_name,
                    np.array(values[0], dtype=self.get_dtype()).reshape(values[1]),
                )
                for param_name, values in params.items()
            )

        return dict(
            (layer_name, to_ndarray(params))
            for layer_name, params in name_to_params.items()
        )
    ...
    def predict_class_local(self, X):
        ...
        :return: a ndarray as the prediction result.
        """
        result = callBigDlFunc(
            self.bigdl_type, "predictLocalClass", self.value, self._to_jtensors(X)
        )
    ...
    def predict_distributed(self, data_rdd, batch_size=-1):
        ...
        """
        result = callBigDlFunc(
            self.bigdl_type, "modelPredictRDD", self.value, data_rdd, batch_size
        )
    ...
    def predict_class_distributed(self, data_rdd):
        ...
        """
        result = callBigDlFunc(
            self.bigdl_type, "modelPredictClass", self.value, data_rdd
        )
        return result
    ...
    def set_weights(self, weights):
        ...
        """
        tensors = [
            JTensor.from_ndarray(param, self.bigdl_type) for param in to_list(weights)
        ]
    ...
    def save_tensorflow(
        self, inputs, path, byte_order="little_endian", data_format="nhwc"
    ):
        """
        ...
        """
        callBigDlFunc(
            self.bigdl_type, "saveTF", self.value, inputs, path, byte_order, data_format
        )
...
class Container(Layer):
    ...
    def flattened_layers(self, include_container=False):
        jlayers = callBigDlFunc(
            self.bigdl_type, "getFlattenModules", self, include_container
        )
        layers = [Layer.of(jlayer) for jlayer in jlayers]
        return layers
...
class Model(Container):
    ...
    def __init__(
        ...
        if jvalue:
            self.value = jvalue
            self.bigdl_type = bigdl_type
        elif model_type == "bigdl" and (
            isinstance(inputs, list) or isinstance(inputs, Node)
        ):
            super(Model, self).__init__(
                None, bigdl_type, to_list(inputs), to_list(outputs)
            )
    ...
    def load_keras(json_path=None, hdf5_path=None, by_name=False):
        ...
        if json_path and not hdf5_path:
            ...
            return WeightLoader.load_weights_from_json_hdf5(
                json_path, hdf5_path, by_name=by_name
            )
    ...
    def load_caffe(model, defPath, modelPath, match_all=True, bigdl_type="float"):
        ...
        :return: A pre-trained model.
        """
        jmodel = callBigDlFunc(
            bigdl_type, "loadCaffe", model, defPath, modelPath, match_all
        )
        return Layer.of(jmodel)
    ...
    def train(
        ...
        rdd_train_sample = rdd_train_images.zip(rdd_train_labels).map(
            lambda input: Sample.from_ndarray(input[0], input[1])
        )
        jmodel = callBigDlFunc(
            bigdl_type,
            "trainTF",
    ...
    def set_input_formats(self, input_formats, bigdl_type="float"):
        ...
        :return:
        """
        jname = callBigDlFunc(bigdl_type, "getRealClassNameOfJValue", self.value)
...
class Attention(Layer):
    ...
    def __init__(self, hidden_size, num_heads, attention_dropout, bigdl_type="float"):
        super(Attention, self).__init__(
            None, bigdl_type, hidden_size, num_heads, attention_dropout
        )
...
class FeedForwardNetwork(Layer):
    ...
    def __init__(self, hidden_size, filter_size, relu_dropout, bigdl_type="float"):
        super(FeedForwardNetwork, self).__init__(
            None, bigdl_type, hidden_size, filter_size, relu_dropout
        )
...
class Sequential(Container):
    ...
    def to_graph(self):
        ...
        """
        jvalue = callBigDlFunc(self.bigdl_type, "toGraph", self.value)
...
class BinaryTreeLSTM(Layer):
    ...
    def __init__(
        ...
        super(BinaryTreeLSTM, self).__init__(
            None, bigdl_type, input_size, hidden_size, gate_output, with_graph
        )
...
class TemporalMaxPooling(Layer):
    ...
    def __init__(self, k_w, d_w=-1, bigdl_type="float"):
        super(TemporalMaxPooling, self).__init__(None, bigdl_type, k_w, d_w)
...
class SpatialMaxPooling(Layer):
    ...
    def __init__(
        ...
        super(SpatialMaxPooling, self).__init__(
            None, bigdl_type, kw, kh, dw, dh, pad_w, pad_h, to_ceil, format
        )
...
class Gemm(Layer):
    def __init__(
        self, alpha=1.0, beta=1.0, trans_a=False, trans_b=False, bigdl_type="float"
    ):
...
class SpatialCrossMapLRN(Layer):
    ...
    def __init__(
        ...
        super(SpatialCrossMapLRN, self).__init__(
            None, bigdl_type, size, alpha, beta, k, data_format
        )
...
class SpatialDropout3D(Layer):
    ...
    def __init__(self, init_p=0.5, data_format="NCHW", bigdl_type="float"):
        super(SpatialDropout3D, self).__init__(None, bigdl_type, init_p, data_format)
...
class SpatialDropout2D(Layer):
    ...
    def __init__(self, init_p=0.5, data_format="NCHW", bigdl_type="float"):
        super(SpatialDropout2D, self).__init__(None, bigdl_type, init_p, data_format)
...
class SpatialDropout1D(Layer):
    ...
    def __init__(self, init_p=0.5, bigdl_type="float"):
        super(SpatialDropout1D, self).__init__(None, bigdl_type, init_p)
...
class Dropout(Layer):
    ...
    def __init__(self, init_p=0.5, inplace=False, scale=True, bigdl_type="float"):
        super(Dropout, self).__init__(None, bigdl_type, init_p, inplace, scale)
...
class GaussianNoise(Layer):
    ...
    def __init__(self, stddev, bigdl_type="float"):
        super(GaussianNoise, self).__init__(None, bigdl_type, stddev)
...
class View(Layer):
    ...
    def __init__(self, sizes, num_input_dims=0, bigdl_type="float"):
        super(View, self).__init__(None, bigdl_type, sizes, num_input_dims)
...
class AddConstant(Layer):
    ...
    def __init__(self, constant_scalar, inplace=False, bigdl_type="float"):
        super(AddConstant, self).__init__(None, bigdl_type, constant_scalar, inplace)
...
class BifurcateSplitTable(Layer):
    ...
    def __init__(self, dimension, bigdl_type="float"):
        super(BifurcateSplitTable, self).__init__(None, bigdl_type, dimension)
...
class Bottle(Container):
    ...
    def __init__(self, module, n_input_dim=2, n_output_dim1=INTMAX, bigdl_type="float"):
        super(Bottle, self).__init__(
            None, bigdl_type, module, n_input_dim, n_output_dim1
        )
...
class CAddTable(Layer):
    ...
    def __init__(self, inplace=False, bigdl_type="float"):
        super(CAddTable, self).__init__(None, bigdl_type, inplace)
...
class CAveTable(Layer):
    ...
    def __init__(self, inplace=False, bigdl_type="float"):
        super(CAveTable, self).__init__(None, bigdl_type, inplace)
...
class CMul(Layer):
    ...
    def __init__(self, size, wRegularizer=None, bigdl_type="float"):
        super(CMul, self).__init__(None, bigdl_type, size, wRegularizer)
...
class Clamp(Layer):
    ...
    def __init__(self, min, max, bigdl_type="float"):
        super(Clamp, self).__init__(None, bigdl_type, min, max)
...
class Cosine(Layer):
    ...
    def __init__(self, input_size, output_size, bigdl_type="float"):
        super(Cosine, self).__init__(None, bigdl_type, input_size, output_size)
...
class CrossProduct(Layer):
    ...
    def __init__(self, numTensor=0, embeddingSize=0, bigdl_type="float"):
...
class ELU(Layer):
    ...
    def __init__(self, alpha=1.0, inplace=False, bigdl_type="float"):
        super(ELU, self).__init__(None, bigdl_type, alpha, inplace)
...
class Euclidean(Layer):
    ...
    def __init__(self, input_size, output_size, fast_backward=True, bigdl_type="float"):
        super(Euclidean, self).__init__(
            None, bigdl_type, input_size, output_size, fast_backward
        )
...
class GradientReversal(Layer):
    ...
    def __init__(self, the_lambda=1.0, bigdl_type="float"):
        super(GradientReversal, self).__init__(None, bigdl_type, the_lambda)
...
class HardShrink(Layer):
    ...
    def __init__(self, the_lambda=0.5, bigdl_type="float"):
        super(HardShrink, self).__init__(None, bigdl_type, the_lambda)
...
class HardTanh(Layer):
    ...
    def __init__(
        self, min_value=-1.0, max_value=1.0, inplace=False, bigdl_type="float"
    ):
        super(HardTanh, self).__init__(None, bigdl_type, min_value, max_value, inplace)
...
class Index(Layer):
    ...
    def __init__(self, dimension, bigdl_type="float"):
        super(Index, self).__init__(None, bigdl_type, dimension)
...
class InferReshape(Layer):
    ...
    def __init__(self, size, batch_mode=False, bigdl_type="float"):
        super(InferReshape, self).__init__(None, bigdl_type, size, batch_mode)
...
class JoinTable(Layer):
    ...
    def __init__(self, dimension, n_input_dims, bigdl_type="float"):
        super(JoinTable, self).__init__(None, bigdl_type, dimension, n_input_dims)
...
class SparseJoinTable(Layer):
    ...
    def __init__(self, dimension, bigdl_type="float"):
        super(SparseJoinTable, self).__init__(None, bigdl_type, dimension)
...
class L1Penalty(Layer):
    ...
    def __init__(
        self, l1weight, size_average=False, provide_output=True, bigdl_type="float"
    ):
        super(L1Penalty, self).__init__(
            None, bigdl_type, l1weight, size_average, provide_output
        )
...
class NegativeEntropyPenalty(Layer):
    ...
    def __init__(self, beta=0.01, bigdl_type="float"):
        super(NegativeEntropyPenalty, self).__init__(None, bigdl_type, beta)
...
class LeakyReLU(Layer):
    ...
    def __init__(self, negval=0.01, inplace=False, bigdl_type="float"):
        super(LeakyReLU, self).__init__(None, bigdl_type, negval, inplace)
...
class LookupTableSparse(Layer):
    ...
    def __init__(
        ...
        super(LookupTableSparse, self).__init__(
            None, bigdl_type, n_index, n_output, combiner, max_norm + 0.0, wRegularizer
        )
...
class MM(Layer):
    ...
    def __init__(self, trans_a=False, trans_b=False, bigdl_type="float"):
        super(MM, self).__init__(None, bigdl_type, trans_a, trans_b)
...
class MV(Layer):
    ...
    def __init__(self, trans=False, bigdl_type="float"):
        super(MV, self).__init__(None, bigdl_type, trans)
...
class MapTable(Container):
    ...
    def __init__(self, module=None, bigdl_type="float"):
        super(MapTable, self).__init__(None, bigdl_type, module)
...
class Max(Layer):
    ...
    def __init__(self, dim, num_input_dims=INTMIN, bigdl_type="float"):
        super(Max, self).__init__(None, bigdl_type, dim, num_input_dims)
...
class Mean(Layer):
    ...
    def __init__(self, dimension=1, n_input_dims=-1, squeeze=True, bigdl_type="float"):
        super(Mean, self).__init__(None, bigdl_type, dimension, n_input_dims, squeeze)
...
class Min(Layer):
    ...
    def __init__(self, dim=1, num_input_dims=INTMIN, bigdl_type="float"):
        super(Min, self).__init__(None, bigdl_type, dim, num_input_dims)
...
class MulConstant(Layer):
    ...
    def __init__(self, scalar, inplace=False, bigdl_type="float"):
        super(MulConstant, self).__init__(None, bigdl_type, scalar, inplace)
...
class Narrow(Layer):
    ...
    def __init__(self, dimension, offset, length=1, bigdl_type="float"):
        super(Narrow, self).__init__(None, bigdl_type, dimension, offset, length)
...
class NarrowTable(Layer):
    ...
    def __init__(self, offset, length=1, bigdl_type="float"):
        super(NarrowTable, self).__init__(None, bigdl_type, offset, length)
...
class Normalize(Layer):
    ...
    def __init__(self, p, eps=1e-10, bigdl_type="float"):
        super(Normalize, self).__init__(None, bigdl_type, p, eps)
...
class Padding(Layer):
    ...
    def __init__(self, dim, pad, n_input_dim, value=0.0, n_index=1, bigdl_type="float"):
        super(Padding, self).__init__(
            None, bigdl_type, dim, pad, n_input_dim, value, n_index
        )
...
class PairwiseDistance(Layer):
    ...
    def __init__(self, norm=2, bigdl_type="float"):
        super(PairwiseDistance, self).__init__(None, bigdl_type, norm)
...
class Power(Layer):
    ...
    def __init__(self, power, scale=1.0, shift=0.0, bigdl_type="float"):
        super(Power, self).__init__(None, bigdl_type, power, scale, shift)
...
class RReLU(Layer):
    ...
    def __init__(self, lower=1.0 / 8, upper=1.0 / 3, inplace=False, bigdl_type="float"):
        super(RReLU, self).__init__(None, bigdl_type, lower, upper, inplace)
...
class SpatialSeparableConvolution(Layer):
    ...
    def __init__(
        ...
        super(SpatialSeparableConvolution, self).__init__(
            None,
            bigdl_type,
            n_input_channel,
            n_output_channel,
...
class ReLU6(Layer):
    ...
    def __init__(self, inplace=False, bigdl_type="float"):
        super(ReLU6, self).__init__(None, bigdl_type, inplace)
...
class SReLU(Layer):
    ...
    def __init__(self, input_shape, share_axes=None, bigdl_type="float"):
        super(SReLU, self).__init__(None, bigdl_type, input_shape, share_axes)

    def set_init_method(
        self, tLeftInit=None, aLeftInit=None, tRightInit=None, aRightInit=None
    ):
        callBigDlFunc(
            self.bigdl_type,
            "setInitMethod",
...
class Replicate(Layer):
    ...
    def __init__(self, n_features, dim=1, n_dim=INTMAX, bigdl_type="float"):
        super(Replicate, self).__init__(None, bigdl_type, n_features, dim, n_dim)
...
class RoiPooling(Layer):
    ...
    def __init__(self, pooled_w, pooled_h, spatial_scale, bigdl_type="float"):
        super(RoiPooling, self).__init__(
            None, bigdl_type, pooled_w, pooled_h, spatial_scale
        )
...
class Scale(Layer):
    ...
    def __init__(self, size, bigdl_type="float"):
        super(Scale, self).__init__(None, bigdl_type, size)
...
class SelectTable(Layer):
    ...
    def __init__(self, index, bigdl_type="float"):
        super(SelectTable, self).__init__(None, bigdl_type, index)
...
class SequenceBeamSearch(Layer):
    ...
    def __init__(
        ...
        super(SequenceBeamSearch, self).__init__(
            None,
            bigdl_type,
            vocab_size,
            beam_size,
...
class SoftMax(Layer):
    ...
    def __init__(self, pos=1, bigdl_type="float"):
...
class SoftPlus(Layer):
    ...
    def __init__(self, beta=1.0, bigdl_type="float"):
        super(SoftPlus, self).__init__(None, bigdl_type, beta)
...
class SoftShrink(Layer):
    ...
    def __init__(self, the_lambda=0.5, bigdl_type="float"):
        super(SoftShrink, self).__init__(None, bigdl_type, the_lambda)
...
class SpatialDilatedConvolution(Layer):
    ...
    def __init__(
        ...
        super(SpatialDilatedConvolution, self).__init__(
            None,
            bigdl_type,
            n_input_plane,
            n_output_plane,
...
class SpatialShareConvolution(Layer):
    ...
    def __init__(
        ...
        super(SpatialShareConvolution, self).__init__(
            None,
            bigdl_type,
            n_input_plane,
            n_output_plane,
...
class VolumetricMaxPooling(Layer):
    ...
    def __init__(
        ...
        super(VolumetricMaxPooling, self).__init__(
            None, bigdl_type, k_t, k_w, k_h, d_t, d_w, d_h, pad_t, pad_w, pad_h
        )
...
class SpatialZeroPadding(Layer):
    ...
    def __init__(self, pad_left, pad_right, pad_top, pad_bottom, bigdl_type="float"):
        super(SpatialZeroPadding, self).__init__(
            None, bigdl_type, pad_left, pad_right, pad_top, pad_bottom
        )
...
class SplitTable(Layer):
    ...
    def __init__(self, dimension, n_input_dims=-1, bigdl_type="float"):
        super(SplitTable, self).__init__(None, bigdl_type, dimension, n_input_dims)
...
class Squeeze(Layer):
    ...
    def __init__(self, dim, num_input_dims=INTMIN, bigdl_type="float"):
        super(Squeeze, self).__init__(None, bigdl_type, dim, num_input_dims)
...
class Sum(Layer):
    ...
    def __init__(
        ...
        super(Sum, self).__init__(
            None, bigdl_type, dimension, n_input_dims, squeeze, size_average
        )
...
class Threshold(Layer):
    ...
    def __init__(self, th=1e-6, v=0.0, ip=False, bigdl_type="float"):
        super(Threshold, self).__init__(None, bigdl_type, th, v, ip)
    ...
    def __init__(self, pos, num_input_dims=INTMIN, bigdl_type="float"):
        if isinstance(pos, int):
            ...
            super(Unsqueeze, self).__init__(
                None, bigdl_type, to_list(posList), num_input_dims
            )
        elif isinstance(pos, list):
            super(Unsqueeze, self).__init__(
                None, bigdl_type, to_list(pos), num_input_dims
            )
        else:
...
class Transpose(Layer):
    ...
    def __init__(self, permutations, bigdl_type="float"):
        super(Transpose, self).__init__(None, bigdl_type, permutations)
...
class SpatialSubtractiveNormalization(Layer):
    ...
    def __init__(self, n_input_plane=1, kernel=None, bigdl_type="float"):
        super(SpatialSubtractiveNormalization, self).__init__(
            None, bigdl_type, n_input_plane, JTensor.from_ndarray(kernel)
        )
...
class SpatialWithinChannelLRN(Layer):
    ...
    def __init__(self, size=5, alpha=1.0, beta=0.75, bigdl_type="float"):
        super(SpatialWithinChannelLRN, self).__init__(
            None, bigdl_type, size, alpha, beta
        )
...
class ConvLSTMPeephole3D(Layer):
    ...
    def __init__(
        ...
        super(ConvLSTMPeephole3D, self).__init__(
            None,
            bigdl_type,
            input_size,
            output_size,
...
class ResizeBilinear(Layer):
    ...
    def __init__(
        ...
        super(ResizeBilinear, self).__init__(
            None, bigdl_type, output_height, output_width, align_corner, data_format
        )
...
class Highway(Layer):
    ...
    def __init__(
        ...
        super(Highway, self).__init__(
            None, bigdl_type, size, with_bias, activation, wRegularizer, bRegularizer
        )
...
class NormalizeScale(Layer):
    ...
    def __init__(
        self, p, scale, size, w_regularizer=None, eps=1e-10, bigdl_type="float"
    ):
        super(NormalizeScale, self).__init__(
            None, bigdl_type, p, eps, scale, size, w_regularizer
        )
...
class DetectionOutputSSD(Layer):
    ...
    def __init__(
        ...
        super(DetectionOutputSSD, self).__init__(
            None,
            bigdl_type,
            n_classes,
            share_location,
...
class DetectionOutputFrcnn(Layer):
    ...
    def __init__(
        ...
        super(DetectionOutputFrcnn, self).__init__(
            None, bigdl_type, nms_thresh, n_classes, bbox_vote, max_per_image, thresh
        )
...
class Cropping2D(Layer):
    ...
    def __init__(self, heightCrop, widthCrop, data_format="NCHW", bigdl_type="float"):
        super(Cropping2D, self).__init__(
            None, bigdl_type, heightCrop, widthCrop, data_format
        )
...
class Cropping3D(Layer):
    ...
    def __init__(
        ...
        super(Cropping3D, self).__init__(
            None, bigdl_type, dim1Crop, dim2Crop, dim3Crop, data_format
        )
...
class RoiAlign(Layer):
    ...
    def __init__(
        self, spatial_scale, sampling_ratio, pooled_h, pooled_w, bigdl_type="float"
    ):
        super(RoiAlign, self).__init__(
            None, bigdl_type, spatial_scale, sampling_ratio, pooled_h, pooled_w
        )
...
class Pooler(Layer):
    ...
    def __init__(self, resolution, scales, sampling_ratio, bigdl_type="float"):
        super(Pooler, self).__init__(
            None, bigdl_type, resolution, scales, sampling_ratio
        )
...
def _test():
    ...
    sc = SparkContext(master="local[4]", appName="test layer", conf=create_spark_conf())
    globs["sc"] = sc
    init_engine()
