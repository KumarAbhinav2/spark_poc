...
# from util.engine import get_bigdl_classpath, is_spark_below_2_2
...
class SingletonMixin(object):
    ...
    @classmethod
    def instance(cls, bigdl_type, *args):
...
class JTensor(object):
    ...
    def __init__(self, storage, shape, bigdl_type="float", indices=None):
        ...
        if indices is None:
            ...
            assert isinstance(
                indices, np.ndarray
            ), "indices should be a np.ndarray, not %s, %s" % (
                type(a_ndarray),
                str(indices),
            )
    ...
    def sparse(cls, a_ndarray, i_ndarray, shape, bigdl_type="float"):
        ...
        assert isinstance(
            i_ndarray, np.ndarray
        ), "indices array should be a np.ndarray, not %s" % type(a_ndarray)
        assert (
            i_ndarray.size == a_ndarray.size * shape.size
    ...
    def to_ndarray(self):
        ...
        return np.array(self.storage, dtype=get_dtype(self.bigdl_type)).reshape(
            self.shape
        )  # noqa
...
class Sample(object):
    ...
    def from_ndarray(cls, features, labels, bigdl_type="float"):
        ...
        if isinstance(features, np.ndarray):
            ...
            assert all(
                isinstance(feature, np.ndarray) for feature in features
        ...
        if np.isscalar(labels):  # in case labels is a scalar.
            ...
            assert all(
                isinstance(label, np.ndarray) for label in labels
    ...
    def from_jtensor(cls, features, labels, bigdl_type="float"):
        ...
        if isinstance(features, JTensor):
            ...
            assert all(
                isinstance(feature, JTensor) for feature in features
        ...
        if np.isscalar(labels):  # in case labels is a scalar.
            ...
            assert all(
                isinstance(label, JTensor) for label in labels
            ), "labels should be a list of np.ndarray, not %s" % type(labels)
...
class RNG:
    ...
    def uniform(self, a, b, size):
        return callBigDlFunc(
            self.bigdl_type, "uniform", a, b, size
        ).to_ndarray()  # noqa
...
def init_executor_gateway(sc, bigdl_type="float"):
    callBigDlFunc(
        bigdl_type, "initExecutorGateway", sc, sc._gateway._gateway_client.port
    )
...
def get_bigdl_conf():
    ...
    for p in sys.path:
        if bigdl_conf_file in p and os.path.isfile(p):
            with open(p) if sys.version_info < (3,) else open(
                p, encoding="latin-1"
            ) as conf_file:  # noqa
...
def _java2py(gateway, r, encoding="bytes"):
    if isinstance(r, JavaObject):
        ...
        if clsName == "JavaRDD":
            jrdd = (
                gateway.jvm.org.apache.spark.bigdl.api.python.BigDLSerDe.javaToPython(r)
            )
        ...
        if clsName in _picklable_classes:
            ...
            isinstance(r, (JavaArray, JavaList))
            and len(r) != 0
...
def _to_java_object_rdd(rdd):
    ...
    return rdd.ctx._jvm.org.apache.spark.bigdl.api.python.BigDLSerDe.pythonToJava(
        rdd._jrdd, True
    )
...
def _py2java(gateway, obj):
    ...
    if isinstance(obj, RDD):
        ...
        obj = ListConverter().convert(
            [_py2java(gateway, x) for x in obj], gateway._gateway_client
        )
